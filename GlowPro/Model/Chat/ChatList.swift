...
class ChatList{
    ...
    init(dict: NSDictionary) {
        /* if let driverDict = dict["drivers"] as? [NSDictionary]{
...
class UserChat{
    ...
    let groupId: String
    var chat : Bool
    var arrOrdersDetails : [OrderInChat] = []
    ...
    init(dict: NSDictionary) {
        ...
        if let dictOrders = dict["order"] as? [NSDictionary]{
            for data in dictOrders{
                let objOrder = OrderInChat(dict: data)
                self.arrOrdersDetails.append(objOrder)
            }
        }
    }
}
    ...
    init(dict : NSDictionary) {
        ...
        if let arrReviewData = dict["review"] as? [NSDictionary]{
            for dictData in arrReviewData{
                let objData = NewReviews(dict: dictData)
